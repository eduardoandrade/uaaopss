...
# PATH
parser.add_argument('--D_resume_dir', type=str, default='', metavar='path to resume D')
...
def test(G, D, target_net, dataset, queryloader, galleryloader, epoch, use_gpu, is_test=False, ranks=[1, 5, 10, 20]):
  ...
  with torch.no_grad():
    ...
    if args.ak_type > 0:
      ...
      print("Hits rate, Rank-1: {:.1%}".format(hits[ranks[0]-1]))
      if not is_test:
        return hits
      ...
      if is_test:
        ...
        print("Results ----------")
        print("Before, mAP: {:.1%}, Rank-1: {:.1%}".format(mAP, cmc[ranks[0]-1]))
        print("After , mAP: {:.1%}, Rank-1: {:.1%}".format(new_mAP, new_cmc[ranks[0]-1]))
        if args.usevis: 
        ...
        print("mAP: {:.1%}, Rank-1: {:.1%}".format(new_mAP, new_cmc[ranks[0]-1]))
